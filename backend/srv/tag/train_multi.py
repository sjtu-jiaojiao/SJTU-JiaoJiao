...
# 数据预处理, 删除文件目录下文件
from text_preprocess import PreprocessTextMulti, delete_file
# 模型图
from graph import TextCNNGraph as Graph
# 计算时间
import time

from config import path_model_dir, path_model, path_hyper_parameters, path_fineture
from config import path_good_tags_train, path_good_tags_valid

def train(hyper_parameters=None, rate=1.0):
    if not hyper_parameters:
        hyper_parameters = {
        'len_max': 20,  # 句子最大长度, 固定推荐20-50, bert越长会越慢, 占用空间也会变大, 本地win10-4G设为20就好, 过大小心OOM
        ...
        'model': {'label': 1175,  # 类别数
                  'batch_size': 128,  # 批处理尺寸, 感觉原则上越大越好,尤其是样本不均衡的时候, batch_size设置影响比较大
                  'dropout': 0.5,  # 随机失活, 概率
        ...
        'data':{'train_data': path_good_tags_train,  # 训练数据
                'val_data': path_good_tags_valid,    # 验证数据
                },
    }
